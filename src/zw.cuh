...
void mysgemm_v11(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C, int lda, int ldb, int ldc){
    // lda = M;
    // ldb = K;
    // ldc = M;
    int tx = threadIdx.x;
...
__global__ void matrix_transpose(float *in, float *out, int rows, int cols){
    ...
    // tile[y][x] = in[(in_corner_i + x) + (in_corner_j + y)*rows];
    // __syncthreads();
    // // read from shared mem, coalesced write to global mem:
    // out[(out_corner_i + x) + (out_corner_j + y)*rows] = tile[x][y];
...
void mysgemm_v1(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C, int lda, int ldb, int ldc){
...
#define KS 32
__global__  __launch_bounds__(1024)
void mysgemm_v2(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C, int lda, int ldb, int ldc){
    // int lda = M, ldb = K, ldc = M;
    int tx = threadIdx.x, ty = threadIdx.y;
    int bx = blockIdx.x, by = blockIdx.y;
    A = &A((bx<<5),0);
...
__global__  __launch_bounds__(1024)
void mysgemm_v3(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C, int lda, int ldb, int ldc){
    // int lda = M, ldb = K, ldc = M;
    int tx = threadIdx.x;
    int bx = blockIdx.x, by = blockIdx.y;
...
__global__  __launch_bounds__(1024)
void mysgemm_v4(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C, int lda, int ldb, int ldc){
    // int lda = M, ldb = K, ldc = M;
    int tx = threadIdx.x;
    int bx = blockIdx.x, by = blockIdx.y;
    int row = tx&31, col = tx>>5;
...
__global__  __launch_bounds__(256)
void mysgemm_v5(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C, int lda, int ldb, int ldc){
    // int lda = M, ldb = K, ldc = M;
    int tx = threadIdx.x;
    int bx = blockIdx.x, by = blockIdx.y;
...
__global__  __launch_bounds__(256)
void mysgemm_v6(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C, int lda, int ldb, int ldc){
    // int lda = M, ldb = K, ldc = M;
    int tx = threadIdx.x;
    int bx = blockIdx.x, by = blockIdx.y;
    int row1 = (tx&7)<<2, row2 = row1+1, row3 = row1+2, row4 = row1+3, col = tx>>3;
...
#define vstore(addr,v1)\
    *((float4 *)(addr)) = v1;
__global__  __launch_bounds__(256)
void mysgemm_v7(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C, int lda, int ldb, int ldc){
    // int lda = M, ldb = K, ldc = M;
    int tx = threadIdx.x;
    int bx = blockIdx.x, by = blockIdx.y;
...
#define vstore(addr,v1)\
    *((float4 *)(addr)) = v1;
__global__  __launch_bounds__(256)
void mysgemm_v8(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C, int lda, int ldb, int ldc){
    // int lda = M, ldb = K, ldc = M;
    int tx = threadIdx.x;
    int bx = blockIdx.x, by = blockIdx.y;
    int row_a = (tx&31)<<2, col_a = tx>>5;
